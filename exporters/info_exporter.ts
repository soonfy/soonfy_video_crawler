...
const info = async (film_id) => {
  try {
    ...
    if (!film) {
      ...
      let star = await film.getStarInfo();
      ...
      let mtime_data = await film.getCompany();
      let rel_c = mtime_data.release_company.join('||') || null;
      let make_c = mtime_data.make_company.join('||') || null;

      let sites = (await film.getSites()).map(x => x.csite).join('||') || null;
      let tvs = (await film.getTvs()).map(x => x.csite).join('||') || null;
