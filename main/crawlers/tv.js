...
const crawler = (uri) => __awaiter(this, void 0, void 0, function* () {
    try {
        let result = [], head = ['剧目名称', '剧目链接', '单集名称', '单集链接', 'vid', 'targetid', '发布日期', '播放量', '采集时间'];
        result.push(head);
        let reg_cid = /\#cover\_edit\/([\w\d]+)/, match = uri.match(reg_cid), cid = '';
        if (match) {
            cid = match[1];
        }
        else {
